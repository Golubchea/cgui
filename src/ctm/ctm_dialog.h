...
static void ctm_dialog_init(struct ctm_dialog * dialog)
{

    struct file_browser *browser =&dialog->browser;
    struct media *media=&dialog->media;
...
static void file_browser_delete(struct file_browser *browser)
{
    if (browser->files)
        dir_free_list(browser->files, browser->file_count);
...
static int file_browser_open(struct file_browser *browser, struct nk_context *ctx)
{
    int ret = 0;
    struct media *media = browser->media;
    struct nk_rect total_space;

    if (nk_begin(ctx, "Открыть", nk_rect(50, 50, 600, 300),
                 NK_WINDOW_BORDER|NK_WINDOW_MOVABLE|NK_WINDOW_CLOSABLE|NK_WINDOW_SCALABLE))
    {
        static float ratio[] = {0.35f, NK_UNDEFINED};
        ...
        nk_label(ctx, "файл:", NK_TEXT_LEFT);
        nk_label(ctx, browser->file, NK_TEXT_LEFT);

        nk_layout_row_dynamic(ctx, 20, 1);

        if(nk_button_label(ctx, "Открыть"))
        {
        ...
        nk_group_begin(ctx, "Special", NK_WINDOW_NO_SCROLLBAR);
        {
            ...
            if (nk_button_image_label(ctx, home, "Домашняя папка", NK_TEXT_CENTERED))
                file_browser_reload_directory_content(browser, browser->home);
            if (nk_button_image_label(ctx,desktop,"Рабочий стол",NK_TEXT_CENTERED))
                file_browser_reload_directory_content(browser, browser->desktop);
            if (nk_button_image_label(ctx,computer,"Рабочая папка",NK_TEXT_CENTERED))
                file_browser_reload_directory_content(browser, "/home/user/code/c/basic_window/build");
            nk_group_end(ctx);
        }
                ...
                {size_t n = j + cols;
                    ...
                    for (; j < count && j < n; ++j) {
                        ...
                        if (j < browser->dir_count) {
                            if (nk_button_image_label(ctx, media->icons.directory,  browser->directories[j], NK_TEXT_CENTERED))
                            {
                                index = (int)j;
                            }
                        } else {
                            ...
                            if (nk_button_image_label(ctx, *icon, browser->files[fileIndex], NK_TEXT_CENTERED))
                            {
...
static int file_browser_save(struct file_browser *browser, struct nk_context *ctx)
{
    int ret = 0;
    struct media *media = browser->media;
    struct nk_rect total_space;

    if (nk_begin(ctx, "Сохранить как...", nk_rect(50, 50, 600, 300),
                 NK_WINDOW_BORDER|NK_WINDOW_MOVABLE|NK_WINDOW_CLOSABLE|NK_WINDOW_SCALABLE))
    {
        static float ratio[] = {0.35f, NK_UNDEFINED};
        float spacing_x = ctx->style.window.spacing.x;
        ...
        nk_layout_row_dynamic(ctx, 20, 2);
        nk_label(ctx, "директория для сохранения:", NK_TEXT_LEFT);
        nk_label(ctx, browser->directory, NK_TEXT_LEFT);


        nk_layout_row_dynamic(ctx, 28, 1);
        nk_flags event = nk_edit_string_zero_terminated(ctx,NK_EDIT_BOX|NK_EDIT_AUTO_SELECT, browser->savebuffer, sizeof( browser->savebuffer), 0);
        nk_layout_row_dynamic(ctx, 20, 1);
        nk_button_label(ctx, "сохранить");
...
static void ctm_dialog_listener(struct ctm_window *win,struct ctm_dialog *dialog)
{

    struct nk_context *ctx=win->ctx;
    struct file_browser *browser=&dialog->browser;

    if(browser->visible==true && browser->isSave==false)
    {
        file_browser_open(browser, ctx);
    }
    if(browser->visible==true && browser->isSave==true)
    {
        file_browser_save(browser, ctx);
    }
}

static void ctm_dialog_delete(struct ctm_dialog* dialog)
{

    file_browser_delete(&dialog->browser);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.home.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.directory.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.computer.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.desktop.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.default_file.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.text_file.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.music_file.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.font_file.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.img_file.handle.id);
    glDeleteTextures(1,(const GLuint*)&dialog->media.icons.movie_file.handle.id);
